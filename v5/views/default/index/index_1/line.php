...
<div class="st-slideTab">
    <div class="st-tabnav">
        ...
        {/loop}
        <a href="{$cmsurl}lines/" class="more">{__('更多')}{$channel['line']['channelname']}</a>
    </div>
    {loop $linedest $ld}
    <div class="st-tabcon">
        <ul class="st-cp-list">
            ...
            <li>
                ...
                <div class="js">
                    <a class="tit" href="{$l['url']}" target="_blank" title="{$l['title']}">{if $l['color']}<span style="color:{$l['color']}">{$l['title']}</span>{else}{$l['title']}{/if}</a>
                    <p class="attr">
                        {loop $l['iconlist'] $ico}
