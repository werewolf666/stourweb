...
class Stourweb_View
{
    ...
    public function capture($kohana_view_filename, array $kohana_view_data)
    {
        ...
        {
            //检测缓存目录
            $tpl_cache_dir = CACHE_DIR . 'v5/tplcache/';
            ...
            {
                $to = CACHE_DIR . 'v5/tplcache/' . $this->_templet . '.php';
                $isfileto = is_file($to);
            ...
            {
                $to = CACHE_DIR.'v5/tplcache/' . $this->_innertex_name . '.php';
    ...
    public function compile_template($str)
    {
        ...
        $str = preg_replace("/\{st:(\w+)\s+([^}]+)\}/ie", "self::st_tag_start('$1','$2','" . str_ireplace('\\', '/', $this->_file) . "')", $str); //标签解析
        ...
        if (is_array($data))
        {
            ...
            foreach ($data as $key => $val)
            {
                ...
                else
                {
                    if (strpos($val, '$') === 0)
                    {
                        $str .= "'$key'=>" . Common::remove_xss($val, 0) . ",";
                    }
                    else
                    {
                        $str .= "'$key'=>'" . addslashes(Common::remove_xss($val, 0)) . "',";
                    }
                }
            }
            return $str . ')';
