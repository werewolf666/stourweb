...
class   Controller_Pc_Global_Hotel extends Stourweb_Controller
{
    private $_webid = 0;
    function before()
    {
        Model_Global_Search::check_app_install(2);
       // global $sys_webid;
    ...
    public function action_index()
    {
        ...
        $rankid = Common::remove_xss($this->request->param('rankid')) ;//星级
        $starttime = Common::remove_xss($_GET['starttime']) ;//开始时间
        $endtime = Common::remove_xss($_GET['endtime']) ;//结束时间
        ...
        $search_items = Model_Global_Module_Hotel::get_search_items($route_array);
