...
class   Controller_Pc_Global_Outdoor extends Stourweb_Controller
{
    private $_webid = 0;
    function before()
    {
    ...
    public function action_index()
    {
        ...
        $dayid = Common::remove_xss($this->request->param('dayid')) ;//出游天数
        $priceid = Common::remove_xss($this->request->param('priceid')) ;//价格
        $startcityid=intval($this->request->param('startcityid'));//出发地
        $groupid = intval($this->request->param('groupid'));//圈子
        $bookstatus = intval($this->request->param('bookstatus'));//状态
        ...
        $search_items = Model_Global_Module_Outdoor::get_search_items($route_array);
