...
<head>
    <meta charset="utf-8">
    <title color_head=Cyvz8B >{$params['keyword']}__搜索结果-{$webname}</title>
            ...
            <ul class="search-results-list">
                ...
                <li class="clearfix">
                    ...
                    <div class="info">
                        ...
                        <div class="attr">
                            {loop $a['attrlist'] $kind}
                             <span class="item">{$kind['attrname']}</span>
                            {/loop}

                        </div>
                        <div class="label">
                            <span class="item">{Common::mydate('Y-m-d',$a['modtime'])}发布</span>
                        </div>
                    </div>
                    <div class="sum">
                        <i class="icon"></i>
                        <span class="read"><b class="num">{$a['shownum']}</b>人已阅读</span>
                    </div>
                </li>
                {/loop}
            </ul>
...
<script>
    ...
    function go_search()
    {
        ...
        var url = SITEURL+'query/'+pinyin+'-'+attrlist+'?keyword='+search_keyword;
