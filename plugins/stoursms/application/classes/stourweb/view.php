...
class Stourweb_View {
     ...
     * @return  View
     */
    public static function factory($file = NULL, array $data = NULL,$innertext = NULL,$innertextname)
    {
        return new Stourweb_View($file, $data ,$innertext,$innertextname);
    }
    ...
    public  function capture($kohana_view_filename, array $kohana_view_data)
    {
        ...
        try
        {
            ...
            {
                $to = CACHE_DIR.'stoursms/tplcache/'.$this->_templet.'.php';
                $isfileto = is_file($to);
            ...
            {
                $to =  CACHE_DIR.'stoursms/tplcache/'.$this->_innertex_name.'.php';
     ...
     */
    public function __construct($file = NULL, array $data = NULL,$innertext = NULL,$innertextname)
    {
        if ($file !== NULL)
        {
        ...
        if ($data !== NULL)
        {
            ...
            $this->_data = $data + $this->_data+self::$_global_data;
            self::$_user_data = $this->_data;
        }
        if ($innertext !== NULL)
        {
            $this->_innertext = $innertext;
            $this->_innertex_name = $innertextname;
        }
    }
    ...
    public function render($file = NULL)
    {
        ...
        if (empty($this->_file) && empty($this->_innertext))
        {
            throw new View_Exception('You must set the file or innertext to use within your view before rendering');
        }
    ...
    public function compile_template($str)
    {
        ...
        $str = preg_replace("/\{\{/",'jsstart',$str);
        $str = preg_replace("/\}\}/",'jsend',$str);
        $str = preg_replace("/\{template\s+([^\}]+)\}/", "<?php echo  Stourweb_View::template(\\1);  ?>", $str); //引用模板
        $str = preg_replace("/\{include\s+([^\}]+)\}/", "<?php echo  Stourweb_View::template(\\1);  ?>", $str); //引用模板
        $str = preg_replace("/\{php\s+(.+)\}/", "<?php \\1?>", $str); //php表达式解析
        $str = preg_replace("/\{php\}/", "<?php ", $str); //php语法解析
        $str = preg_replace("/\{\/php\}/", "?>", $str); //php语法解析
        $str = preg_replace("/\{request\s+([^\}]+)\}/","<?php echo Request::factory(\\1)->execute()->body(); ?>",$str);
        ...
        $str = preg_replace("/\{([a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*\:\:[a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*\(([^{}]*)\))\}/", "<?php echo \\1;?>", $str); //函数解析Common::func($pa,$pa2)静态函数
        ...
        $str = preg_replace("/\{st:(\w+)\s+([^}]+)\}/ie", "self::st_tag_start('$1','$2','".$this->_file."')", $str); //标签解析
    ...
    public static function st_tag_start($op, $data,$kohana_view)
    {
        ...
        {
            $includestr = 'require_once ("'.$viewpath.'/taglib/'.$op.EXT.'");';
            $hastagfile = 1;
        ...
        elseif(file_exists(TAGPATH.$op.EXT))
        {
            ...
            $includestr = 'require_once ("'.$pub_tag_path.$op.EXT.'");';
            $hastagfile = 1;
        ...
        if (is_array($data))
        {
            ...
            foreach ($data as $key=>$val)
            {
                ...
                else
                {
                    if (strpos($val, '$')===0)
                    {
                        $str .= "'$key'=>".Common::remove_xss($val).",";
                    }
                    else
                    {
                        $str .= "'$key'=>'".addslashes(Common::remove_xss($val))."',";
                    }
                }
            }
            return $str.')';
