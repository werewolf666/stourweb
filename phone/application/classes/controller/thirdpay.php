...
class Controller_Thirdpay extends Stourweb_Controller{
...

    {
        ...
        $kq_merchantAcctId =  Common::get_sys_conf('value','cfg_bill_account',0)."01";   //*  商家用户编号		(30)

        $kq_key = Common::get_sys_conf('value','cfg_bill_key',0);

        $kq_inputCharset	= "1";	//   1 ->  UTF-8		2 -> GBK		3 -> GB2312   default: 1	(2)

        $kq_pageUrl	    = $GLOBALS['cfg_basehost'].$GLOBALS['cfg_phone_cmspath']."/thirdpay/bill_return";	//   直接跳转页面	(256)

        $kq_bgUrl	    = $GLOBALS['cfg_basehost'].$GLOBALS['cfg_phone_cmspath']."/thirdpay/bill_notify";	//   后台通知页面	(256)
...
_FRM_;
             ...
             switch($_REQUEST['payResult']){
                 ...
                 case '10':
                     ...
                     */
                     ...
                     $rtnUrl=$GLOBALS['cfg_basehost'].$GLOBALS['cfg_phone_cmspath']."/thirdpay/paysuccess";
        ...
        switch($_REQUEST['payResult'])

            {
                ...
                case '10':
					...
					$trade_price = intval($_REQUEST['payAmount'])/100;



                    $this->pay_success($ordersn,$paySource,$trade_price);
...

    {
       ...
       $GLOBALS['cfg_alipay_pid'] = Common::get_sys_conf('value','cfg_alipay_pid',0); //pid

       $GLOBALS['cfg_alipay_key'] = Common::get_sys_conf('value','cfg_alipay_key',0); //key

       $GLOBALS['cfg_alipay_account'] = Common::get_sys_conf('value','cfg_alipay_account',0); //帐号

       include(VENDORPATH.'/thirdpay/alipay/alipay.config.php');

       include(VENDORPATH.'/thirdpay/alipay/lib/alipay_submit.class.php');
    ...
    //服务器异步通知页面路径
        ...
        $notify_url = $GLOBALS['cfg_basehost'].$GLOBALS['cfg_phone_cmspath']."/thirdpay/alipay_notifyurl";
   ...
   //页面跳转同步通知页面路径
        ...
        $call_back_url = $GLOBALS['cfg_basehost'].$GLOBALS['cfg_phone_cmspath']."/thirdpay/alipay_backurl";
   ...
   //操作中断返回地址
        ...
        $merchant_url =$GLOBALS['cfg_basehost'].$GLOBALS['cfg_phone_cmspath']."/pub/payerror";
...
//建立请求
...

    {
        ...
        include(VENDORPATH.'/thirdpay/alipay/lib/alipay_notify.class.php');
...

        {
           ...
           /*  if ($alipay_config['sign_type'] == '0001')
            ...
            if( ! empty($doc->getElementsByTagName( "notify" )->item(0)->nodeValue) ) {
				...
				$trade_price = $doc->getElementsByTagName( "total_fee" )->item(0)->nodeValue;
...
*/     
        ...
        $GLOBALS['cfg_wxpay_appid'] = Common::get_sys_conf('value','cfg_wxpay_appid',0); //appid

        $GLOBALS['cfg_wxpay_mchid'] = Common::get_sys_conf('value','cfg_wxpay_mchid',0); //mchid

        $GLOBALS['cfg_wxpay_key'] = Common::get_sys_conf('value','cfg_wxpay_key',0); //key

        $GLOBALS['cfg_wxpay_appsecret'] = Common::get_sys_conf('value','cfg_wxpay_appsecret',0); //secret



        include_once(VENDORPATH.'/thirdpay/weixinpay/WxPayPubHelper/WxPayPubHelper.php');
       ...
       //使用jsapi接口
        ...
        $this->display('pub/weixinpaypost');
...
*/  
    ...
    include_once(VENDORPATH.'/thirdpay/weixinpay/log_.php');
    ...
    $log_->log_result("【接收到的notify通知】:\n".$xml."\n");
...

        {
            ...
            //此处应该更新一下订单状态，商户自行增删操作
				...
				$trade_price = intval($notify->data['total_fee']) / 100;
    ...
    * */
...

    {
        ...
        $this->request->redirect('pub/paysuccess');
	...
	public function pay_success($ordersn,$paySource,$payPrice=null)

	{
        ...
        $configs=$configModel->get_config(0);

        $totalPrice=0;

        $totalPrice = intval($arr['dingnum'])*intval($arr['price'])

            +intval($arr['childnum'])*intval($arr['childprice'])

            +intval($arr['oldnum'])*intval($arr['oldprice']);

        if($arr['typeid']==2)
        {
            $totalPrice=Model_Hotel::suit_range_price($arr['suitid'], $arr['usedate'], $arr['departdate'], $arr['dingnum']);
            $totalPrice = $totalPrice * $arr['dingnum'];
        }
...

        {
            ...
            $msgInfo = Product::get_define_msg($arr['typeid'],3);
            ...
            $orderAmount = Product::calculate_price($arr);
...

            {
                ...
                Product::send_msg($memberInfo['mobile'],$nickname,$content);
            ...
            $emailInfo=Product::get_email_msg($arr['typeid'],3);
...

            {
...

                {
                    ...
                    Product::add_jifen_log($arr['memberid'],"预订{$arr['productname']}获得积分{$addjifen}",$addjifen,2);
