...
ST.Util = {
    ...
    showBox: function (title, url, boxwidth, boxheight, closefunc, nofade, fromdocument, params) {
        var doc = fromdocument ? fromdocument : top.document;
        if (boxwidth === 0 ) {
            boxwidth = doc.body.clientWidth * 0.8;
        }
        if (boxheight === 0 ) {
            boxheight = doc.body.clientHeight * 0.78;
        }
    ...
    helpBox: function (ele, label, e) {
        ...
        this.showBox('云端帮助', SITEURL + 'user/dialog_help?label=' + label, 600, 300, null, null, document, {loadWindow: window});
    },
    dialogHelpBox: function (ele, label, e) {
        if (e && e.stopPropagation) {
            e.stopPropagation();
        }
        else {
        ...
        // parent.window.floatBox('帮助', 'user/dialog_help', 600,300, null, null, document, {loadWindow: window});
        this.showBox('云端帮助', 'user/dialog_help?label=' + label, 600, 300, null, null, document, {loadWindow: window});
    },
    getGridHelp: function (label) {
        return '<img class="ml-5" title="查看帮助' + label + '" style="cursor:pointer; vertical-align: middle; margin-top: -3px;" src="'+window.PUBLICURL+'images/help-ico.png" onclick="ST.Util.helpBox(this,\'' + label + '\',event)">';
    },
    ...
    resizeDialog: function (selector, hasWidth) {
        var dialog = this.getDialog();
        var maxHeight = dialog.maxHeight;
        var maxWidth = dialog.maxWidth;
        var height = $(selector).height();
    ...
    goClone: function (id, product) {
        ST.Util.confirmBox('克隆产品', '<div class="add-class"><span class="set-text-xh" style="border: none;padding-right: 5px;">克隆条数:</span><input type="text" id="cloneNum" class="set-text-xh text_100 mt-2 title-color" value="" placeholder="请输入条数"/></div>', function () {
            var num = $("#cloneNum", window.parent.document).val();
            num = parseInt(num);
            if (num > 0) {
                $.ajax({
                    type: 'POST',
                    async: false,
                    url: SITEURL + product + '/ajax_clone',
                    data: {id: id, num: num},
    ...
    getNextDestSet: function (ele, pid, status, kindlist, productid) {
        ...
        Ext.Ajax.request({
            ...
            success: function (response, opts) {
                ...
                var del_i = step;
                while (true) {
                    var remove_div = Ext.select("#dest_set_" + productid + " .dest-set-list-" + del_i);
                    if (remove_div.getCount() <= 0)
                        break;
                    else
                        remove_div.remove();
                    del_i++;
...
ST.Modify = {
    ...
    getUploadFile: function (arr, showsethead) {
        ...
        $.each(arr, function (i, item) {
            ...
            html += '<div class="pic"><img src="' + item.litpic + '" /></div>';
            html += '<p class="p1">';
