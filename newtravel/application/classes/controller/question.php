	...
	public function action_index()
	{
		...
		else if($action=='read')    //读取列表
		{
            ...
            if(isset($this->params['typeid'])){
                $w.=' and typeid='.$this->params['typeid'];
            }
			...
			foreach($list as $k=>$v)
			{
                ...
                $v['productname'] =$v['questype']==0? ORM::factory('question')->getProductName($v['productid'],$v['typeid']):"<a target='_blank' class='product-title' href='/questions/'>{$v['title']}</a>";
                $new_list[] = $v;
			}
