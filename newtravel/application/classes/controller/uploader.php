...
class Controller_Uploader extends Stourweb_Controller
{
    ...
    public function action_uploadfile()
    {
        ...
        $icon = ARR::get($_POST, 'icon');//验证是否是图标
        ...
        if ($icon)
        {
            $iconarr = array('/uploads/' . $pinyin . '/litico/' . date('Ymd'), 33, 15);
            array_push($storearr, $iconarr);
        }
            ...
            if (Upload::size($file, "2M"))
            {
                if (Upload::type($file, array('jpg', 'png', 'gif')))
                {
                    ...
                    if (Upload::save($file, $filename))
                    {
                        ...
                        if ($icon)
                        {//判断返回的路径
                            $arr['litpic'] = $GLOBALS['$cfg_basehost'] . $storearr[3][0] . '/' . $filename;
                        }
                        else
                        {
    ...
    public function action_delpicture()
    {
        ...
        $tablename = ARR::get($_POST, 'tablename');
        $id = ARR::get($_POST, 'destid');
        //Common::deleteRelativeImage($pic,$tablename,$id);
    ...
    public function action_certs()
    {
        ...
        if ($rs = move_uploaded_file($filedata['tmp_name'], $filepath))
        {
            ...
            $distDirs = Model_Config::is_exists_certs($configinfo, true);
            //解压文件
            ...
            foreach ($distDirs[$method] as $v)
            {
                $unzippath = BASEPATH . $v;
                if (file_exists($unzippath) || substr_count($v, 'payment'))
                {
