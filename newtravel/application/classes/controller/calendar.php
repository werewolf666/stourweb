...
class Controller_Calendar extends Stourweb_Controller
{
    ...
    {
        if (isset(self::$priceTableArr[$typeid]))
            return self::$priceTableArr[$typeid];
        $model = ORM::factory('model', $typeid);
        if ($model->loaded() && $model->maintable == 'model_archive')
        {
            return 'model_suit_price';
        }
    }
