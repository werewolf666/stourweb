...
class Controller_Attrid extends Stourweb_Controller
{
    ...
    */
    ...
    public function action_modelattr()
    {
        ...
        if (empty($action))
        {
            ...
            $r = DB::select('pinyin','correct')->from('model')->where('id','=',$typeid)->execute()->current();
            $pinyin = $r['correct'] ? $r['correct'] : $r['pinyin'];
    ...
    */
    public static function getattridlist($typeid)
    {
        ...
        $w = $tablename == 'model_attr' ? " and typeid='$typeid'" : '';
    ...
    public function action_extendlist()
    {
        ...
        else if ($action == 'delete') //删除某个记录
        {
            ...
            if (is_numeric($id)) //
            {
                ...
                //清理扩展字段
                DB::delete('model_content')->where('columnname','=',$fieldname)->and_where('typeid','=',$typeid)->execute();
            }
        }
    ...
    public function action_content()
    {
        ...
        else if ($action == 'save')
        {
            ...
            //更新扩展字段描述
            Model_Model_Content::update_extend_field_name($typeid);
            echo 'ok';
        }
    }
    ...
    public function addContentField($typeid, $field, $description)
    {
        ...
        $content_table = $modeuleinfo['pinyin'] . '_content';
        $table = $content_table;
        if ($modeuleinfo['issystem'] != 1 || empty($table))
        {
            $isTongyong = true;
            $table = 'model_content';
        }
