...
class Controller_Destination extends Stourweb_Controller
{
    ...
    public function action_destination()
    {
        ...
        if (empty($action))
        {
            $jmp_destid = $_GET['jmp_destid'];
            if(!empty($jmp_destid))
            {
                $jmp_destname = DB::select('kindname')->from('destinations')->where('id','=',$jmp_destid)->execute()->get('kindname');
                $this->assign('jmp_destname',$jmp_destname);
            }
            ...
            $addmodule = ORM::factory('model')->where('maintable', '=', 'model_archive')->get_all(); //扩展模型
            ...
            $this->display($typeid == 0 ? 'stourtravel/destination/destination' : 'stourtravel/destination/destination_tongyong');
        }
        else if ($action == 'read')
        {
            ...
            else
            {
                ...
                $sql = "select a.id,a.kindname,if(find_in_set($typeid,opentypeids),1,0) as isopen,a.pinyin,a.iswebsite,{$bfields} from sline_destinations a left join $product_dest_table b on a.id=b.kindid where a.pid=$pid and a.isopen=1";
            ...
            if ($typeid == 0) //只有主目的地有添加功能.
            {
                $list[] = array(
                    'leaf' => true,
                    'id' => $pid . 'add',
                    'kindname' => '<button class="btn btn-primary size-S radius" onclick="addSub(' . $pid . ')">添加</button>',
        ...
        else if ($action == 'update')
        {
            ...
            if ($typeid == 0)
            {
                ...
                {
                    $ar = explode('.', $val);
                    $py = str_replace(St_Functions::get_http_prefix(), '', $ar[0]);
                ...
                //开启子站
                if ($field == 'iswebsite' && $val > 0)
                {
                    //子站伪静态
                    $fileHtaccess = BASEPATH . DIRECTORY_SEPARATOR . '.htaccess';
                    if (file_exists($fileHtaccess))
                    {
                        $weburl = DB::select()->from('weblist')->where('webid', '=', 0)->order_by('id', 'ASC')->execute()->current();
                        $hostname = str_replace(St_Functions::get_http_prefix() . $weburl['webprefix'], '', $weburl['weburl']);
                        $prefix = $weburl['webprefix'];
                        $subhtaccess = "RewriteCond %{HTTP_HOST} ^(.*)?{$hostname}\$\r\nRewriteCond %{HTTP_HOST} !^({$prefix}){$hostname}\$\r\nRewriteRule ^(lines|hotels|cars|spots|raiders|photos)(/)?\$ \\$1/%1 [L]";
                        $htaccess = file_get_contents($fileHtaccess);
                        if (stripos($htaccess, '#substation') !== false)
                        {
                            $htaccess = preg_replace('~#substation start.*?#substation end~is', "#substation start\r\n{$subhtaccess}\r\n#substation end", $htaccess);
                        }
                        else
                        {
                            $htaccess = preg_replace('~RewriteBase\s+/~', "RewriteBase /\r\n#substation start\r\n{$subhtaccess}\r\n#substation end", $htaccess);
                        }
                        file_put_contents($fileHtaccess, $htaccess);
                    }
                    //子站广告位
                    Model_Advertise_5x::substation($id);
                }
                //保存数据
                $model->save();
                if ($model->saved())
        ...
        else if ($action == 'save')
        {
            ...
            if ($typeid == 0)
            {
                ...
                $old_pinyin = $dest->pinyin;
                if ($field)
                {
                    if ($field == 'kindname' && $data->$field!=$dest->$field)
                    {
                        $org_pinyin = $dest->pinyin;
                        $dest->pinyin = empty($current_pinyin) ? Common::getPinYin($data->$field) : $current_pinyin;
                        if ($org_pinyin != $dest->pinyin)
                        {
                            $dest->pinyin = Model_Destinations::get_legal_pinyin($dest->pinyin);
                        }
                        $pinyin = $dest->pinyin;
                    }
                    $dest->$field = $data->$field;
                    if ($dest->pinyin != $old_pinyin)
                    {
                        $dest->pinyin = Model_Destinations::get_legal_pinyin($dest->pinyin);
                        $pinyin = $dest->pinyin;
                    }
                ...
                $dest_chk_model = ORM::factory('destinations')->where("pinyin='{$dest->pinyin}' and id!={$dest->id}")->find();
        ...
        else if ($action == 'addsub')
        {
            ...
            $dest->pid = $pid;
            $dest->kindname = "未命名";
            $dest->pinyin = Common::getPinYin($dest->kindname);
            $dest->pinyin = Model_Destinations::get_legal_pinyin($dest->pinyin);
            $dest->save();
            if ($dest->saved())
            {
                $dest->reload();
                $dest->update_sibling('add');
    ...
    public function action_ajax_before_destroy()
    {
        $data = $_POST;
        if (isset($data['id']) && count($data['id']))
        {
            $result = DB::select()->from('destinations')->where('pid', 'in', DB::expr('(' . implode(',', $data['id']) . ')'))->execute()->current();
    ...
    */
    public function action_ajax_getDestsetList()
    {
        ...
        $w = !empty($typeid) ? " and find_in_set({$typeid},opentypeids)" : '';

        if ($keyword)
        {
            $sql = "select id,kindname,pinyin from sline_destinations where kindname like '%{$keyword}%' and isopen=1 {$w}  order by pinyin asc";
        }
        else
        {
            $sql = "select id,kindname,pinyin from sline_destinations where pid=$pid and isopen=1 {$w} order by pinyin asc";
        }
        $destlist = DB::query(Database::SELECT, $sql)->execute()->as_array();
        foreach ($destlist as $key => $row)
        {
            $sql = "select count(*) as num from sline_destinations where pid='{$row['id']}' and isopen=1 {$w}";
    ...
    public function action_dialog_attrdest()
    {
        ...
        $this->assign('id', $id);
        $this->assign('typeid', $typeid);
        $this->display('stourtravel/destination/dialog_attrdest');
    }
