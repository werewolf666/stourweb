...
class Controller_App extends Stourweb_Controller
{
    ...
    public function action_topusernav()
    {
        ...
        if (empty($action))
        {
            $configinfo = DB::select()->from('sysconfig')->where('varname', '=', 'cfg_usernav_open')->where('webid', '=', 0)->execute()->current();
    ...
    //删除
    public function action_ajax_before_delete()
    {
        $bool = false;
        foreach ($_POST['id'] as $id)
        {
            $status = DB::select('id')->from('plugin_leftnav')->where('pid', '=', (int)$id)->execute()->current();
            if ($status)
            {
                $bool = true;
                break;
        ...
        echo json_encode(array('status' => !$bool ? true : false));
    }
    ...
    public function action_mutititle()
    {
        ...
        $product_list = DB::select()->from('model')->where('is_showable', '=', '1')->execute()->as_array();
        foreach ($product_list as $key => $row)
        {
            $kindlist_table = $row['pinyin'] . '_kindlist';
            if (!St_Database::is_table_exists('sline_' . $kindlist_table))
            {
                unset($product_list[$key]);
                continue;
            }
            $seo_title_name = 'cfg_' . $row['pinyin'] . '_title';
            $seo_description_name = 'cfg_' . $row['pinyin'] . '_desc';

            $product_list[$key]['seo_title_name'] = $seo_title_name;
            $product_list[$key]['seo_description_name'] = $seo_description_name;
            $product_list[$key]['seo_title_value'] = $out[$seo_title_name];
            $product_list[$key]['seo_description_value'] = $out[$seo_description_name];
    ...
    public function action_ajax_mutititle_save()
    {
        ...
        {
            $result = DB::select()->from('plugin_autotitle')->where('name', '=', $k)->execute()->current();
            ...
            {
                DB::insert('plugin_autotitle', array('name', 'value'))->values(array($k, $v))->execute();
            }
        }
        self::rewriteCache();
        echo 'ok';
