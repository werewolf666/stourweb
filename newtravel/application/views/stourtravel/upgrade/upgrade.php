...
<scirpt id="task_load_task_data" type="text/html" style="display: none;">
    ...
    <br/>
    <br/>
    <p class="tiwen">正在加载任务数据与检测升级环境，请稍候...</p>
...
<scirpt id="task_load_task_data_exec_error" type="text/html" style="display: none;">
    ...
    <br/>
    <br/>
    <p class="error">加载任务数据与检测升级环境失败，原因：{{#error_msg}}</p>
</scirpt>
...
<scirpt id="major_upgrade_confirm" type="text/html" style="display: none;">
    <br/>
    <br/>
    <br/>
    <br/>
    <br/>
    <p class="tiwen">此次升级包含重大版本更新，影响的功能较多，可能存在未知的升级风险，建议联系思途售后协助完成升级，您确定要现在立即升级吗？</p>
    <a class="cancel" href="javascript:show_task_cancel_ui();">放弃升级</a>
    <a class="sure" href="javascript:exec_upgrade_patch();">立即升级</a>
</scirpt>
...
<script language="JavaScript">
    ...
    var is_betaupgrade = {$is_betaupgrade};
        ...
        $.ajax({
            ...
            success: function (rs) {
                if (rs.status == 1) {
                    ...
                    if (rs.msg == "major_upgrade_confirm") {
                        patch_list = rs.data;
                        current_patch_index = 0;

                        show_major_upgrade_confirm_ui();
                    }
                    else {
    ...
    function show_load_task_data_exec_error_ui(error_msg) {
    ...
    function show_major_upgrade_confirm_ui() {
        var html = template('major_upgrade_confirm', null);
        $('#upgrade_info').html(html);
    }
