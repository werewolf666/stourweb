       ...
       <table class="list-head-tb">
        ...
        <div class="search-bar filter" id="search_bar">
            ...
            <div class="pro-search ml-10" style=" float:left; margin-top:4px">
                <input type="text" id="searchkey" value="农家乐名称/农家乐编号" datadef="农家乐名称/农家乐编号" class="sty-txt1 set-text-xh wid_200">
...
<script>
  ...
  Ext.onReady(
    function() 
    {
        ...
        $("#addbtn").click(function(){
            var url=SITEURL+"farm/add/parentkey/member/itemid/1";
            ST.Util.addTab('添加农家乐','{$cmsurl}farm/add/parentkey/product/itemid/2',0);
        });
        ...
        window.product_store=Ext.create('Ext.data.Store',{
         ...
         proxy:{
		   type:'ajax',
		   api: {
              read: SITEURL+'farm/list/action/read',  //读取数据的URL
			  update:SITEURL+'farm/list/action/save',
			  destroy:SITEURL+'farm/list/action/delete'
              },
		      reader:{
                type: 'json',   //获取数据的格式 
                root: 'lists',
	   ...
	   columns:[
			   ...
			   },
			   {
				   ...
				   renderer : function(value, metadata,record) {
					              var id=record.get('id'); 
								   if(id.indexOf('suit')!=-1)
								        metadata.tdAttr ="data-qtip='指同一农家乐下套餐的显示顺序'"+"data-qclass='dest-tip'";
                                  var newvalue=value;
               ...
               },
			   {
				   ...
				   renderer : function(value, metadata,record) {
							    ...
							    var id=record.get('id');
                                var roomid = record.get('suitid');
                                var farmid = record.get('farmid');
                                ...
                                var url=record.get('url');
			                         ...
			                         else if(id.indexOf('suit')!=-1)
									 {
									   ...
									   return "<a href='javascript:void(0);' onclick=\"modSuit('"+roomid+"','"+farmid+"')\" class='suit-title'>&bull;&nbsp;&nbsp;"+value+"</a>";
									 }
						}
           ...
           },
           {
           ...
           },
			   {
				   ...
				   renderer : function(value, metadata,record) {
						 ...
						 if(id.indexOf('suit')!=-1)
						   {
							   return "<a href='javascript:;' class='row-mod-btn' title='编辑' onclick=\"modSuit('"+suitid+"','"+farmid+"')\"></a>&nbsp;&nbsp;<a href='javascript:;' title='删除' class='row-del-btn' onclick=\"delSuit('"+id+"')\"></a>";
						   }
					     else
                             return '<a href="javascript:;"  class="row-add-suit-btn" onclick="ST.Util.addTab(\'添加套餐\',\'farm/addsuit/parentkey/product/itemid/2/action/add/farmid/'+id+'\')">添加套餐</a>';
  ...
  {
      var url = SITEURL+'farm/edit/parentkey/product/itemid/2/id/'+id;

      parent.window.addTab('修改农家乐',url,0);
  }
  //房型管理
  function roomList(id,farmname)
  {
      var url = SITEURL+'farm/room/parentkey/product/itemid/2/farmid/'+id;
      ST.Util.addTab(farmname,url);

  }
//modSuit
  function modSuit(suitid,farmid)
  {
      //var hotelid = $("#hotelid").val();
      var url = SITEURL+'farm/editsuit/parentkey/product/itemid/2/action/edit/farmid/'+farmid+'/suitid/'+suitid;
      ST.Util.addTab('修改套餐',url);
  }
