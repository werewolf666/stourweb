       ...
       <div class="main-body">
           <div class="nav">
               <div class="w-set-tit bom-arrow">
                   ...
                   <span data-rel="picture"><s></s>图片管理{Common::get_help_icon('destination_dialog_basicinfo_images',true)}</span>
           ...
           <div class="nav-list">
               <div class="item-one" id="item_jieshao" style="display: none;">
                   ...
                   <table>
                       <tr><td class="tit">介绍{Common::get_help_icon('destination_dialog_basicinfo_jieshao',true)}：</td><td  class="u-editor">  {php Common::getEditor('jieshao',$info['jieshao'],500,180);}</td></tr>
                   </table>
               </div>
               <div class="item-one" id="item_seo">
                   <table>
                       <tr><td class="tit">拼音{Common::get_help_icon('destination_dialog_basicinfo_pinyin',true)}：</td><td><input type="text" name="pinyin" class="set-text-xh text_250" value="{$info['pinyin']}"/></td></tr>
                       <tr><td class="tit">优化标题{Common::get_help_icon('destination_dialog_basicinfo_seotitle',true)}：</td><td><input type="text" name="seotitle" class="set-text-xh text_250" value="{$info['seotitle']}" /></td></tr>
                       <tr><td class="tit">Tag词{Common::get_help_icon('destination_dialog_basicinfo_tagword',true)}：</td><td><input type="text" name="tagword" class="set-text-xh text_250" value="{$info['tagword']}"/></td></tr>
                       <tr><td class="tit">关键词{Common::get_help_icon('destination_dialog_basicinfo_keyword',true)}：</td><td><input type="text" name="keyword" class="set-text-xh text_250" value="{$info['keyword']}"/></td></tr>
                       <tr><td class="tit">页面描述{Common::get_help_icon('destination_dialog_basicinfo_description',true)}：</td><td><textarea name="description" class="des">{$info['description']}</textarea></td></tr>
                   </table>
               </div>
               <div class="item-one" id="item_picture" style="display: none;">
                   <table>
                       ...
                       <tr><td><div id="up_btn" class="uploadify" style="height: 27px; width: 80px;"><div id="up_btn-button" class="uploadify-button " style="text-indent: -9999px; height: 27px; line-height: 27px; width: 80px;"><span class="uploadify-button-text">SELECT FILES</span></div></div><span class="tip-img-size tip-img-padding0">建议上传尺寸640*435px</span></td></tr>
                   </table>
               </div>
               <div class="item-one" id="item_template" style="display: none;">
                   <table>
                       <tr><td class="tit">显示模板{Common::get_help_icon('destination_dialog_basicinfo_templetpath',true)}：</td><td><a href="javascript:;" data-rel="" class="i-tpl {if empty($info['templetpath'])}on{/if}">标准</a>
    ...
    $(function() {
       ...
       $('#up_btn').click(function(){
        ...
        $(document).on('click','.confirm-btn',function(){
            //  拼音检测
            var pinyin = $(".item-one input[name=pinyin]").val();
            if( !pinyin || pinyin.trim().length <= 0)
            {
                ST.Util.showMsg('拼音不能为空!','5',2000);
                return;
            }
            var reg = /^[a-zA-Z]{1}([a-zA-Z0-9]){0,19}$/;
            var flag = reg.test(pinyin);
            if(! flag)
            {
                ST.Util.showMsg('拼音必须英文字母开头并且只含英文字母和数字!','5',3000);
                return;
            }
            var data={};
            data['pinyin']= pinyin;
