...
<head>
    <meta charset="utf-8">
    <title>模块页面配置-思途CMS{$coreVersion}</title>
...
$(function(){
    ...
    $(".web-set").find('a').each(function(i,obj){
           ...
           $(obj).click(function(){
               ...
               if(webid!=0){
                   var url = 'sitetemplet/index/site/'+webid+'/parentkey/templet/menuid/{$_GET["menuid"]}';
                   ST.Util.addTab(webname,url);
               }else{
                   var url = 'templet/index/parentkey/templet/menuid/{$_GET["menuid"]}';
                   ST.Util.addTab(webname,url);
               }
