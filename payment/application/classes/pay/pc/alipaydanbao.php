...
/**
 * PC 端支付宝担保交易
 * Class Pay_Pc_AlipayDanBao
 */
class Pay_Pc_AlipayDanBao
{
    //支付宝即时交易目录
    private $_alipayDanBaoDir;
    //异步通知
    const NOTIFY_URL = '/callback/index/Pay_Pc_AlipayDanBao-notify_url/';
    //同步通知
    const ERTURN_URL = '/callback/index/Pay_Pc_AlipayDanBao-return_url/';

    public function __construct()
    {
        $this->_alipayDanBaoDir = Common::C('interface_path') . 'pc/alipay_danban/';
    }

    public function submit($data)
    {
        require_once($this->_alipayDanBaoDir . "lib/alipay_submit.class.php");
        //支付宝配置
    ...
    public function _alipay_config()
    {
        ...
        $alipay_config['cacert'] = $this->_alipayDanBaoDir . 'cacert.pem';
    ...
    {
        $bool = 'fail';
        include($this->_alipayDanBaoDir . 'lib/alipay_notify.class.php');
        $alipay_config = $this->_alipay_config();
        ...
        if ($result)
        {
            ...
            if ($_POST['trade_status'] == 'TRADE_FINISHED')
            {
                ...
                {
                    $method = Common::C('pc');
                    Common::pay_success($_POST['out_trade_no'], $method['method']['13']['name']);
                }
                $bool = 'success';
            }
            else
