...
class Pay_Pc_Bill
{
    ...
    //异步通知
    const NOTIFY_URL = '/callback/index/Pay_Pc_Bill-notify_url/';
    //同步通知
    const ERTURN_URL = '/callback/index/Pay_Pc_Bill-return_url/';
    ...
    {
        $html = '<form name="kqPay" method="GET" action="https://www.99bill.com/gateway/recvMerchantInfoAction.htm" accept-charset="utf-8">';
    ...
    public function notify_url()
    {
        if ($this->_verify())
        {
            ...
            {
                $tip = '信息:快钱(异)交易,订单金额与实际支付不一致';
                if (Common::total_fee_confirm($_REQUEST['orderId'], $_REQUEST['payAmount'] / 100, $tip))
                {
                    $method = Common::C('pc');
    ...
    public function return_url()
    {
        ...
        {
            $info['sign'] = '22';
            new Pay_Exception("状态:快钱交易数据有效性验证失败");
        }
