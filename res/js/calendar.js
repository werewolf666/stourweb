...
var currency_symbol=typeof(CURRENCY_SYMBOL)== "undefined"?'￥':CURRENCY_SYMBOL;
function Calendar(year,month) {
                        ...
                        for (var itr = 0; itr < itinerary.data.length; itr++) {
                            ...
                            if (_day == day && month == _month && _year == year) {
                               ...
                               // var flag = time1 < time2; //判断当前日历日期是否小于当前日期加上提前预定天数
									...
									for(var li = 0; li < infoArr.length; li++)
									{
										...
										for(var ci = 0; ci < comDay.length; ci++)
										{
											...
											if(cDateD.getTime() >= sDate.getTime() && cDateD.getTime() <= jDate.getTime())
											{
												if(diffd >= comDay[ci])
												{
													price = itinerary.data[itr].price - comPrice[ci];
													break;
												}
												else
												{
													price = itinerary.data[itr].price;
												}
											}
											else
											{
								...
								var childprice =itinerary.data[itr].childprice;
                                var tempDateTime = year + "-" + (month < 10 ? "0" + month : month) + "-" + (day < 10 ? "0" + day : day); //2012-08-02 JoJo 考虑值的获取与传递 标签自定义属性date >>exists 是否存在切位于后台使用
								
                                if (flag) {
									var pricehtml = price == "0" ? "电询" : currency_symbol + price;
                                    innerHTML = textContext = "<div datetime='" + tempDateTime + "' exists='false'>" + dayT + day + "<div><font class='calendar_color_333'>&nbsp;<b>过期</b>" + itinerary.data[itr].description + "</font></div></div>";
                                }
                                else {
                                    var thhtml ='';
                                    if(price.length>0){
                                    ...
                                    var thhtml = "<div id='d" + itr + "' " + (this.isWeb == true ? "" : "") + " datetime='" + tempDateTime + "' exists='true' onmouseover='divchange1(this)' onmouseout='divchange2(this)' onclick=\"setBeginTime("+year+","+month+","+day+","+price+","+$lineid+","+$id+")\">" + dayT + day + "<div><a style=\"display:block;text-align:center;\">" + itinerary.data[itr].description + "</a><a class='calendar_red'  >" + pricehtml + "</a></div></div>";
                                    }
    ...
    this.show = function(ele, id, url, iseven, ischangecss, isweb,lineid) {
        ...
        if (this.even && ele) {
            ...
            new Calendar($this.year,$this.month).show(ele, id, url, "", $this.isChangeCss, $this.isWeb,lineid);
			//if($('#calenderPartEven').find('table').length<2)
			//{
				ele.appendChild(ui);
			//}
           // alert(parseInt(this.month)+1);
            var next_month_year = this.month==12? parseInt(this.year)+1:this.year;
            var next_month = this.month ==12? 1: parseInt(this.month)+1;

            $this.change(next_month_year, next_month, $this.date, 1, idarr[2]);
...
function Calendar_GetPdateAndPrice_New(ele, id, url, iseven, ischangecss, isweb,func,lineid) {
    ...
    if (isweb!=undefined && isweb == false) {
        ...
        $.ajax({
            type: "post",
            url: SITEURL+"line/ajax_line_suit_price",
			data: "suitid="+suitid+"&lineid="+lineid,
            dataType:'json',
            success: function(msg) {
                  if(msg.data.length==0){
                      msg={"data":[ { "pdatetime": "2016-06-24", "price": "","childprice": "","description": "", "info": ""}]}
                      $("#zxyd").remove();
                  }
				Calendar_itinerary = msg; //转换为json对象
                var temp=Calendar_itinerary.data[0].pdatetime.split('-');
                var _this= new Calendar(temp[0],temp[1]);
                _this.show(ele, id, url, iseven,ischangecss,isweb,lineid);
                func();
            }
        });
    }
...
function divchange1(obj) {
    $(obj).css({ "background-color": "#FCEFA0", "cursor": "pointer" });
...
function showDiv(obj, hotelid, tid, lineid)
{
	...
	else
	{
		...
		Calendar_GetPdateAndPrice(document.getElementById("calenderPartEven"), id,"",true);
	}
}
